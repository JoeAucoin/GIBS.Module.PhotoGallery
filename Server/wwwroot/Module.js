document.addEventListener("DOMContentLoaded", function () {
  ...
  function getOriginalItems() {
    return Array.from(document.querySelectorAll(".multi-carousel-item:not(.clone)"));
  }

  function initializeClones() {
    const originalItems = getOriginalItems();

    document.querySelectorAll(".multi-carousel-item.clone").forEach((clone) => clone.remove());
  ...
  function setCarouselHeight() {
    ...
    const carouselHeight = Math.min(Math.max(availableHeight, 300), 340);
  ...
  let totalItems = getOriginalItems().length;
  if (totalItems === 0) {
    return;
  }
  ...
  window.addEventListener("resize", function () {
    ...
    if ((wasMobile && itemsPerSlide > 1) || (!wasMobile && itemsPerSlide === 1)) {
      initializeClones();
      totalItems = getOriginalItems().length;
